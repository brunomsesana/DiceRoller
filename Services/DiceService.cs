...
namespace myapp.Services
{
    public class DiceService
    {
        ...
        public ResultadoDTO Roll(string macro){
            ...
            return new ResultadoDTO(resultadoFinal, _rollLogs);
        }
        ...
        private static int EncontrarFechamento(string texto, int startIndex)
        {
            int contador = 1;
        ...
        private Rolagem? RolarDados(string macro)
        {
            ...
            foreach (var (valor, index) in todasAsRolagens)
            {
                if (indicesMantidos.Contains(index))
                {
                    resultadosFormatados.Add(valor.ToString());
                    soma += valor;
                }
                else
                {
                    resultadosFormatados.Add($"~{valor}~");
                }
            }
